...
function TaskRow({ item, color, onToggle, onDelete, onEdit, t }) {
  ...
  return (
    <div ref={ref} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{ position:"relative", display:"flex", alignItems:"center", gap:10, padding:"10px 6px", borderBottom:"1px solid #F3EFF8", transition:"opacity 0.25s" }}>
      {sparks.map(s=><Sparkle key={s.id} color={s.color} x={s.x} y={s.y}/>)}
      <button onClick={handleToggle} style={{ width:24, height:24, borderRadius:8, flexShrink:0, border:`2.5px solid ${color}`, background:item.done?color:"transparent", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center", transition:"background 0.2s, transform 0.1s", transform:hov&&!item.done?"scale(1.1)":"scale(1)", opacity:item.done?0.5:1, position:"relative", zIndex:1 }}>
        {item.done && <span style={{ color:"#fff", fontSize:13, fontWeight:900, lineHeight:1 }}>✓</span>}
      </button>
      <span style={{ position:"absolute", left:0, right:0, textAlign:"center", fontSize:14, color:"#0F0E2A", fontWeight:500, textDecoration:item.done?"line-through":"none", opacity:item.done?0.45:1, transition:"opacity 0.25s", pointerEvents:"none" }}>{item.title}</span>
      <div style={{ flex:1 }}/>
...
function CheckRow({ item, color, onToggle, onDelete, t }) {
  ...
  return (
    <div ref={ref} onMouseEnter={()=>setHov(true)} onMouseLeave={()=>setHov(false)}
      style={{ position:"relative", display:"flex", alignItems:"center", gap:10, padding:"9px 6px", borderBottom:"1px solid #F3EFF8", transition:"opacity 0.25s" }}>
      {sparks.map(s=><Sparkle key={s.id} color={s.color} x={s.x} y={s.y}/>)}
      ...
      <span style={{ position:"absolute", left:0, right:0, textAlign:"center", fontSize:14, color:"#0F0E2A", fontWeight:500, textDecoration:item.checked?"line-through":"none", opacity:item.checked?0.45:1, transition:"opacity 0.25s", pointerEvents:"none" }}>{item.title}</span>
      <div style={{ flex:1 }}/>
      {hov && <button onClick={()=>onDelete(item.id)} style={{ background:"none",border:"1px solid #FFE0E0",borderRadius:6,cursor:"pointer",fontSize:11,padding:"3px 8px",color:"#FF8080",fontWeight:700,fontFamily:"inherit", position:"relative", zIndex:1 }}>{t("btn_delete")}</button>}
    </div>
  );
}
